          ...
          <div>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {/* ── Per-person summary panel (non-משמרת, visible to all) ── */}
                {!isShiftSec && (() => {
                  const ONCALL_HRS = { 'כ': 6, 'כמ': 6, 'כש': 10, 'כמש': 10 };
                  const activePeople = people.filter(p => !p.includes('תקן') && !p.includes('נוסף'));
                  if (!activePeople.length) return null;
                  const yearEntries = Object.entries(days).filter(([iso]) => iso.startsWith(String(year)));
                  const stats = activePeople.map(person => {
                    let mOncall = 0, yOncall = 0, mAway = 0, yAway = 0;
                    for (const { iso } of monthDays) {
                      const code = (days[iso] || {}).statuses?.[person] || '';
                      mOncall += ONCALL_HRS[code] || 0;
                      if (classifyStatus(code) === 'away') mAway++;
                    }
                    for (const [, dayData] of yearEntries) {
                      const code = dayData.statuses?.[person] || '';
                      yOncall += ONCALL_HRS[code] || 0;
                      if (classifyStatus(code) === 'away') yAway++;
                    }
                    return { person, mOncall, yOncall, mAway, yAway };
                  });
                  const isMe = p => p === myName;
                  return (
                    <div style={{ marginTop: 12, background: 'rgba(255,255,255,0.015)', border: `1px solid ${sc.accent}22`, borderRadius: 12, padding: '12px 14px' }}>
                      {/* Header */}
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                        <div style={{ width: 6, height: 6, borderRadius: '50%', background: sc.accent }} />
                        <span style={{ fontSize: 11, fontWeight: 700, color: sc.accent, letterSpacing: 0.4 }}>סיכום {MONTHS_HE[selMonth]} · {year}</span>
                      </div>
                      {/* Column headers */}
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr repeat(4, 56px)', gap: 4, marginBottom: 6, paddingBottom: 6, borderBottom: `1px solid rgba(255,255,255,0.07)` }}>
                        <div style={{ fontSize: 9, color: '#445' }} />
                        <div style={{ textAlign: 'center', fontSize: 9, color: '#e67e22', fontWeight: 700, lineHeight: 1.2 }}>כוננות<br/>חודש</div>
                        <div style={{ textAlign: 'center', fontSize: 9, color: '#e67e2299', fontWeight: 700, lineHeight: 1.2 }}>כוננות<br/>שנה</div>
                        <div style={{ textAlign: 'center', fontSize: 9, color: '#16a085', fontWeight: 700, lineHeight: 1.2 }}>שתפ״א<br/>חודש</div>
                        <div style={{ textAlign: 'center', fontSize: 9, color: '#16a08599', fontWeight: 700, lineHeight: 1.2 }}>שתפ״א<br/>שנה</div>
                      </div>
                      {/* Per-person rows */}
                      {stats.map(({ person, mOncall, yOncall, mAway, yAway }) => (
                        <div key={person} style={{ display: 'grid', gridTemplateColumns: '1fr repeat(4, 56px)', gap: 4, padding: '5px 0', borderBottom: '1px solid rgba(255,255,255,0.04)', alignItems: 'center', background: isMe(person) ? 'rgba(74,158,255,0.04)' : 'transparent', borderRadius: 4 }}>
                          <div style={{ fontSize: 12, fontWeight: isMe(person) ? 700 : 400, color: isMe(person) ? '#4a9eff' : '#ccd6f6', paddingRight: 4, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{person}</div>
                          {/* כוננות month */}
                          <div style={{ textAlign: 'center' }}>
                            {mOncall > 0
                              ? <span style={{ fontSize: 13, fontWeight: 700, color: '#e67e22' }}>{mOncall}<span style={{ fontSize: 9, color: '#667', marginRight: 1 }}>ש׳</span></span>
                              : <span style={{ fontSize: 11, color: '#334' }}>—</span>}
                          </div>
                          {/* כוננות year */}
                          <div style={{ textAlign: 'center' }}>
                            {yOncall > 0
                              ? <span style={{ fontSize: 12, fontWeight: 600, color: '#a0622a' }}>{yOncall}<span style={{ fontSize: 8, color: '#556', marginRight: 1 }}>ש׳</span></span>
                              : <span style={{ fontSize: 11, color: '#334' }}>—</span>}
                          </div>
                          {/* שתפ"א month */}
                          <div style={{ textAlign: 'center' }}>
                            {mAway > 0
                              ? <span style={{ fontSize: 13, fontWeight: 700, color: '#16a085' }}>{mAway}<span style={{ fontSize: 9, color: '#667', marginRight: 1 }}>י׳</span></span>
                              : <span style={{ fontSize: 11, color: '#334' }}>—</span>}
                          </div>
                          {/* שתפ"א year */}
                          <div style={{ textAlign: 'center' }}>
                            {yAway > 0
                              ? <span style={{ fontSize: 12, fontWeight: 600, color: '#0d7363' }}>{yAway}<span style={{ fontSize: 8, color: '#556', marginRight: 1 }}>י׳</span></span>
                              : <span style={{ fontSize: 11, color: '#334' }}>—</span>}
                          </div>
                        </div>
                      ))}
